...
class CustomChartWidget(QWidget):
    ...
    def __init__(
        ...
        # Buffers — dùng deque để pop phần đầu O(1), convert sang numpy khi plot
        ...
        # FIX #3: cache Y-range để tránh setRange() thừa
        self._last_temp_ymax: float | None = None
        self._last_pres_ymax: float | None = None
        self._Y_CHANGE_THRESH = 0.02   # 2% thay đổi mới update

        # FIX #1: cache X-range để tránh setXRange() thừa
        self._last_x_min: float | None = None
        self._last_x_max: float | None = None
        self._X_CHANGE_THRESH = 0.5    # 0.5 s thay đổi mới update
        ...
        # Khởi tạo X-range một lần lúc đầu (không cần timer nữa)
        self._update_axis()
    ...
    def _create_legend(self):
        ...
        for i in range(self.num_temp):
            w = self._build_temp_legend(temp_names[i], TEMP_COLORS[i], font_legend, i)
        ...
        if self.num_temp > 0 and self.num_pressure > 0:
            ...
            for i in range(self.num_pressure):
                w = self._build_pressure_legend(pressure_names[i], PRESSURE_COLORS[i], font_legend, i)
                legend_bar.addWidget(w)
    ...
    def _on_legend_click(self, kind: str, index: int):
        self._solo = None if self._solo == (kind, index) else (kind, index)
    ...
    def _create_pressure_viewbox(self):
        ...
        axis_right = FixedAxis(orientation="right")
    ...
    def _create_temp_labels(self):
        self._temp_labels = []
        for i in range(self.num_temp):
            item = pg.TextItem(text="", color=TEMP_COLORS[i], anchor=(0, 0.5), border=None)
    ...
    def _create_pressure_labels(self):
        ...
        for i in range(self.num_pressure):
            item = pg.TextItem(text="", color=PRESSURE_COLORS[i], anchor=(0, 0.5), border=None)
            item.setFont(QFont(self._font_family, FONT_SIZE_SM, QFont.Weight.Bold))
            item.fill = pg.mkBrush(0, 0, 0, 0)
            item.hide()
    ...
    # ── FIX #1: X-range update — không còn timer riêng ───────────────────────
    def _update_axis(self):
        """
        Cập nhật X-range.
        Được gọi trong append_data() thay vì QTimer độc lập.
        Có cache để skip setXRange() nếu thay đổi < 0.5 s.
        """
        now       = time.time()
        ...
        # Chỉ setRange khi thay đổi đáng kể
        if (
            self._last_x_min is None
            or abs(x_min - self._last_x_min) > self._X_CHANGE_THRESH
        ):
    ...
    # ── FIX #3: Y-range với threshold guard ──────────────────────────────────
    def _update_y_ranges(self):
        # Temperature
        temp_values = [v for arr in self._ty for v in arr]
        if temp_values:
            t_max      = max(temp_values)
            top_pad    = max(t_max * 0.1, 1)
            bottom_pad = max(t_max * 0.03, 2)
            new_ymax   = t_max + top_pad

            prev = self._last_temp_ymax
            if prev is None or abs(new_ymax - prev) / max(abs(prev), 1) > self._Y_CHANGE_THRESH:
                self.plot.setRange(yRange=(-bottom_pad, new_ymax), padding=0)
                self._last_temp_ymax = new_ymax

        # Pressure
        if self.num_pressure > 0:
            pres_values = [v for arr in self._py for v in arr]
            if pres_values:
                p_max    = max(pres_values)
                top_pad  = max(p_max * 0.25, 1)
                new_ymax = p_max + top_pad
    ...
    def append_data(
        ...
        # FIX #2: convert sang numpy array 1 lần, tránh tạo list() trung gian
        for i in range(self.num_temp):
            ...
            self._temp_curves[i].setData(
                np.fromiter(self._tx[i], dtype=np.float64, count=len(self._tx[i])),
                np.fromiter(self._ty[i], dtype=np.float64, count=len(self._ty[i])),
            )
        ...
        for i in range(self.num_pressure):
            ...
            self._pressure_curves[i].setData(
                np.fromiter(self._px[i], dtype=np.float64, count=len(self._px[i])),
                np.fromiter(self._py[i], dtype=np.float64, count=len(self._py[i])),
            )

        # FIX #1: update X ngay tại đây, không cần timer độc lập
        self._update_axis()
    ...
    def set_temp_range(self, y_min: float, y_max: float):
        ...
        self._last_temp_ymax = None  # reset cache
    ...
    def set_pressure_range(self, y_min: float, y_max: float):
        ...
        self._last_pres_ymax = None  # reset cache
